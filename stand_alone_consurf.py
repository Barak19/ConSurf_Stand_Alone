    ...
    if searchType == "HMMER":
        ...
        while line != "":
            ...
            if match1:
                ...
                regex = r"^\s*" + re.escape(seq_name) + r"\s*(\d+)\s+(\S+)\s+(\d+)"
...
def run_search(Search_Out_File):
    ...
    if form['DNA_AA'] == "AA":
        ...
        if form['Homolog_search_algorithm'] == "BLAST":
            ...
            cmd = "mmseqs createdb  %s query.fa\nmmseqs search query.fa %s alnNew temp -a 1 --max-seqs 10000000 -s 5.7 --min-seq-id %.2f --max-seq-id %.2f -c %s --filter-hits 1  -e %s\nmmseqs convertalis query.fa %s alnNew %s --format-output \"target,evalue,taln,tstart,tend,pident,theader\"\n" %(vars['protein_seq'], vars['protein_db'], float(form['MIN_IDENTITY']) / 100, float(form['MAX_REDUNDANCY']) / 100, vars['hit_min_length'], form['E_VALUE'], vars['protein_db'], Search_Out_File)
